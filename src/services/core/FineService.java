...
public class FineService {
    ...
    public void calculateAndUpdateAllFines() {
        ...
        ValidationResult userIdResult = userValidator.validateField("userId", currentUser.getUserId());
    ...
    private void setUserFinesDirectly(String userId, float amount) {
        // Validation
        UserValidator userValidator = validatorFactory.getUserValidator();
        BorrowValidator borrowValidator = validatorFactory.getBorrowValidator();

        ValidationResult result = new ValidationResult();

        ValidationResult userIdResult = userValidator.validateField("userId", userId);
        if (userIdResult.hasErrors()) {
            result.getErrors().addAll(userIdResult.getErrors());
        }

        ValidationResult fineResult = borrowValidator.validateFineAmount(amount);
        if (fineResult.hasErrors()) {
            result.getErrors().addAll(fineResult.getErrors());
        }

        if (result.hasErrors()) {
            System.err.println("Fine validation failed:");
            result.printErrors();
            return;
        }
